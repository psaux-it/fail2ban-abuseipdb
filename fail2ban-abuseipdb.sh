...
#   avoid relying on Fail2Ban's internal ban state and instead use a separate, isolated AbuseIPDB
#   SQLite database as the single source of truth for what has already been reported.
#
#   `norestored` must stay at 0 (Fail2Ban default) — restart and duplicate-report handling is
#   controlled entirely at the script level via 'BYPASS_FAIL2BAN', not through Fail2Ban's own
#   restored-ticket mechanism. See 'abuseipdb.local' to toggle this behavior.
#
#   The script performs two API calls per ban action:
...
#   AbuseIPDB's 15-minute per-IP report cooldown is also tracked locally to skip redundant
#   calls and avoid HTTP 429 errors on repeat offenders.
#
#   Concurrency is bounded by a 'flock'-based worker pool ('MAX_WORKERS'), so simultaneous bans
#   never overwhelm the AbuseIPDB API or cause SQLite write contention; a separate per-IP lock
#   also prevents the same IP from being processed twice by overlapping Fail2Ban triggers.
#   'actionstart' self-heals the local database on every run (integrity check, schema migration)
#   and blocks 'actionban' outright if initialization ever fails. If a report call fails, the
#   IP is rolled back out of the local database so it will be retried on the next ban.
#
...
#       /etc/fail2ban/action.d/fail2ban-abuseipdb.sh "your_api_key" "Failed SSH login attempts" "192.0.2.1" "18" "600"
#
# Arguments:
#   $1 APIKEY          - Required (Core). Retrieved automatically from the Fail2Ban 'jail'.        | Your AbuseIPDB API key.
#   $2 COMMENT         - Required (Core). Retrieved automatically from the Fail2Ban 'jail'.        | Custom, per-jail report comment; also prevents leakage of sensitive data when reporting
